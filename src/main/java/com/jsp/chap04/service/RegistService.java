...
public class RegistService implements IDancerService{
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
        ...
        // POST 방식에서 전달된 한글 데이터의 깨짐 방지 메서드 (한글이 깨져서 나오기때문에 깨짐방지!!)
        request.setCharacterEncoding("UTF-8");

        // 제공된 form 에 작성된 사용자의 입력값 가져오기
        String name = request.getParameter("name");
        String crewName = request.getParameter("crewName");
        String danceLevel = request.getParameter("danceLevel");
        // checkbox 처럼 여러 값을 전달받는 경우에는 getParameterValues 를 사용합니다 -> String 배열로 리턴됩니다.
        String[] genresArray = request.getParameterValues("genres");

        // 얻어온 입력값을 save()에게 전달 -> 객체 생성 및 Map 에 저장까지 일괄처리
        DancerRepository.save(name, crewName, danceLevel, genresArray);

        ///////////////////////////////// 등록은 여기까지 /////////////////////////////////////////

        // 등록이 완료 되었으므로, 지금까지 등록된 댄서의 목록을 화면에 보여주고 싶다.
        // MVC 구조에서 화면응답은 View 가 전담하기 때문에, 서비스는 repository 로 부터 목록을 받아와서
        ...
        // request 에 담자
        // serAttribute("이름", 저장할 값) -> 이름을 통해 값을 얻을 수 있습니다.
